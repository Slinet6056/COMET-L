...
/**
 * Deep code analyzer for RAG knowledge extraction. Analyzes code patterns, control flow, and
 * dependencies.
 */
public class DeepAnalyzer {
  ...
  public String analyzeDeep(String filePath) throws Exception {
    ...
    CompilationUnit cu =
        parseResult.getResult().orElseThrow(() -> new RuntimeException("Parse result is empty"));
  ...
  private JsonObject analyzeMethod(MethodDeclaration method, ClassOrInterfaceDeclaration cls) {
    ...
    method
        .getJavadocComment()
        .ifPresent(javadoc -> methodInfo.addProperty("javadoc", javadoc.getContent()));
            ...
            ifStmt -> {
              ...
              checks.forEach(
                  check -> {
                    ifStmt
                        .getRange()
                        .ifPresent(range -> check.addProperty("line", range.begin.line));
            ...
            binary -> {
              ...
              if (op == BinaryExpr.Operator.LESS
                ...
                if (leftStr.contains(".length")
                    || rightStr.contains(".length")
                    || leftStr.contains(".size()")
                    || rightStr.contains(".size()")) {
              ...
              tryStmt
                  ...
                  .forEach(
                      catchClause -> {
                        ...
                        catchInfo.addProperty(
                            "exceptionType", catchClause.getParameter().getTypeAsString());
                        catchInfo.addProperty(
                            "paramName", catchClause.getParameter().getNameAsString());
  ...
  private JsonArray analyzeFieldAccess(MethodDeclaration method, ClassOrInterfaceDeclaration cls) {
    ...
    cls.getFields()
        .forEach(
            field -> field.getVariables().forEach(var -> classFields.add(var.getNameAsString())));
        ...
        .forEach(
            nameExpr -> {
              ...
              if (classFields.contains(name)) {
                ...
                nameExpr
                    .getRange()
                    .ifPresent(range -> access.addProperty("line", range.begin.line));
    ...
    if (parent.isPresent()) {
      ...
      if (p instanceof com.github.javaparser.ast.expr.AssignExpr) {
        com.github.javaparser.ast.expr.AssignExpr assign =
            (com.github.javaparser.ast.expr.AssignExpr) p;
  ...
  private boolean isWriteAccess(Expression expr) {
    ...
    if (parent.isPresent()) {
      ...
      if (p instanceof com.github.javaparser.ast.expr.AssignExpr) {
        ...
        com.github.javaparser.ast.expr.UnaryExpr unary =
            (com.github.javaparser.ast.expr.UnaryExpr) p;
        // ++, --, etc.
  ...
  /**
   * Calculate cyclomatic complexity of a method. CC = E - N + 2P, simplified as: 1 + number of
   * decision points
   */
